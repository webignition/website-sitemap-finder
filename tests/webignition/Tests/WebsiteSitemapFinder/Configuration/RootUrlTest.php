...
class RootUrlTest extends ConfigurationTest {
    ...
    public function testGetReturnsValueSet() {
        $this->assertEquals(self::ROOT_URL, (string)$this->configuration->setRootUrl(self::ROOT_URL)->getRootUrl());
    }
    
    public function testGetReturnsNormalizedUrlObject() {        
        $this->assertInstanceOf('webignition\NormalisedUrl\NormalisedUrl', $this->configuration->setRootUrl(self::ROOT_URL)->getRootUrl());
    }
}